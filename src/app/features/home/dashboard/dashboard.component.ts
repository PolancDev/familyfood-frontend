...
export class DashboardComponent implements OnInit {
  ...
  readonly isInvitado = computed(() => this.authService.userRole() === 'INVITADO');

  readonly cards = computed<DashboardCard[]>(() => {
    const role = this.authService.userRole();
    const hasFamily = this.familyService.families().length > 0;
    ...
    const allCards: DashboardCard[] = [
      ...
      {
        title: 'Mi Familia',
        description: 'Gestiona los miembros de tu grupo, revisa solicitudes de acceso y organiza tu familia.',
        icon: 'pi pi-users',
    ...
    // "Botón del Pánico" solo visible para ADMIN
    if (role === 'ADMIN') {
      allCards.push({
    ...
    // Solo mostrar "Configurar Familia" si el usuario es INVITADO (no tiene familia)
    if (role === 'INVITADO' || !hasFamily) {
      allCards.push({
